...
const StatsPage: React.FC = () => {
  ...
  const totalApplications = roles.reduce((sum, role) => sum + role.total, 0);
  const totalOffers = roles.reduce((sum, role) => sum + role.offer, 0);
  const totalRejected = roles.reduce((sum, role) => sum + role.rejected, 0);
  const totalActive = roles.reduce((sum, role) => sum + role.active, 0);
  const totalIgnored = roles.reduce((sum, role) => sum + role.ignored, 0);
  const totalAbandoned = roles.reduce((sum, role) => sum + role.abandoned, 0);
  ...
  return (
    <div className="stats-page">
          ...
          <div className="stats-page__overview-row">
            ...
            <div className="stats-page__overview-card stats-page__overview-card--danger">
              ...
              <div className="stats-page__overview-content">
                ...
                <span className="stats-page__overview-label">Отклонено</span>
              </div>
            </div>
          </div>
          ...
          <div className="stats-page__overview-row">
            ...
            <div className="stats-page__overview-card stats-page__overview-card--neutral">
              ...
              <div className="stats-page__overview-content">
                ...
                <span className="stats-page__overview-label">Заброшено</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="stats-page__charts">
        <h2 className="stats-page__charts-title">Статистика по должностям</h2>
